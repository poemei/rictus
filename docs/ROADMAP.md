...
Use the IRC module as the first real Core integration fixture. The IRC capability is now behind the module boundary without rewriting the proven protocol code. Next prove the enriched `.so`/`.dll` artifacts still build, then use Core loading/lifecycle activation to replace the temporary direct Core startup path. Core remains responsible for qualification and explicit human enablement.
